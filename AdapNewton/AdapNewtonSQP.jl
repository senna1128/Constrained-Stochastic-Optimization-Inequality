...
function AdapNewtonSQP(nlp,sigma,Max_Iter,EPS_Step,EPS_Res,alpha_max,eta,epsilon,delta,beta,rho,kap_grad,kap_f,p_grad,p_f,C_grad)
    ...
    while eps>EPS_Step && KKT[end]>EPS_Res && k<Max_Iter
        ...
        if ((bnab_augL_k - bnab_augL_k1)'SQPDir)[1]>eta/4*norm([SQPDir[1:nx];J_1;J_2c])^2 || IdSing == 1
            # Compute Hessian
            Hxx = Diagonal(ones(nx))+eta*G_k'G_k+1/epsilon*G_k[1:necon,:]'G_k[1:necon,:] + 1/(epsilon*q_nuxlam)*G_k[act_set.+necon,:]'G_k[act_set.+necon,:]
            diagTerm = [zeros(necon);c_k[IdUpp:end].^2]
            diagTerm[act_set.+necon] = zeros(length(act_set))
            Term1 = Diagonal(diagTerm)
            Term2 = zeros(ncon,nx)
            Term2[Equ_Act,:] = G_k[Equ_Act,:]
            Hmlx = Term2 + eta*(G_k*G_k'+Term1)*G_k
            Term3 = [zeros(necon); -epsilon*q_nuxlam*ones(nicon)]
            Term3[act_set.+necon] = zeros(length(act_set))
            Hmlml = Diagonal(Term3) + eta*(G_k*G_k'+Term1)^2
            Hhat = hcat(vcat(Hxx,Hmlx),vcat(Hmlx', Hmlml))
            Hhat = Hhat+(0.1 + opnorm(Matrix(Hhat)))*Diagonal(ones(nx+ncon))
            try
