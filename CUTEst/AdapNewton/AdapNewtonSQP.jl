...
function AdapNewtonSQP(nlp,sigma,kap,cerr,cgrad,Max_Iter,EPS_Step,EPS_Res,epsilon,delta,eta,beta,alpha_max,rho,kap_grad,kap_f,chi_grad,chi_f,p_grad,p_f,Id_Mul=0)
...
#        println("Iter--",Iter,"-KKT-",KKT[end])  # debug
        ...
        if IdSing1==1 || ((bnab_augL_k-bnab_augL_k1)'SQPDir)[1]>eta/4*norm([SQPDir[1:nx];J_1;J_2c])^2
            ...
            Term3 = G_k*G_k'+Term1
            Hmlx = Term2 + eta*Term3*G_k
            Term4 = [zeros(necon);-epsilon*q_nuxlam*ones(nicon)]
            Term4[act_set.+necon] = zeros(length(act_set))
