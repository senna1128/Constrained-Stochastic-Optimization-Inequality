...
function AdapGDMain(AdapGD, Prob)
    ...
    # tuning
    Kappa,chi_err,C_grad = AdapGD.Kappa,AdapGD.chi_err,AdapGD.C_grad
    Sigma = AdapGD.Sigma
    ...
    for Id_Prob = 1:length(Prob)
        ...
        for Id_Sig = 1:LenSigma
            Id_Rep = 1
            while Id_Rep <= Rep
                println("AdapGD-",Id_Prob,"-Default-Sig-",Id_Sig,"-Rep-",Id_Rep)
                KKT,CountF,CountG,CountH,Alpha,Time,FETri,FEOLDTri,NSRatio,FSRatio,Id_Con = AdapGDSQP(nlp,Sigma[Id_Sig],kap,cerr,cgrad,Max_Iter,EPS_Step,EPS_Res,epsilon,delta,eta,beta,alpha_max,rho,kap_grad,kap_f,chi_grad,chi_f,p_grad,p_f,0)
                if Id_Con != 1
                    Id_Rep += 1
                else
        ...
        for Id_Cgrad = 2:LenTest[1]
            for Id_Sig = 1:LenSigma
                Id_Rep = 1
                while Id_Rep <= Rep
                    println("AdapGD-",Id_Prob,"-Cgrad-",Id_Cgrad,"-Sig-",Id_Sig,"-Rep-",Id_Rep)
                    KKT,CountF,CountG,CountH,Alpha,Time,FETri,FEOLDTri,NSRatio,FSRatio,Id_Con = AdapGDSQP(nlp,Sigma[Id_Sig],kap,cerr,C_grad[Id_Cgrad],Max_Iter,EPS_Step,EPS_Res,epsilon,delta,eta,beta,alpha_max,rho,kap_grad,kap_f,chi_grad,chi_f,p_grad,p_f,0)
                    if Id_Con != 1
                        Id_Rep += 1
                    else
        ...
        for Id_Kap = 2:LenTest[2]
            for Id_Sig = 1:LenSigma
                Id_Rep = 1
                while Id_Rep <= Rep
                    println("AdapGD-",Id_Prob,"-Kap-",Id_Kap,"-Sig-",Id_Sig,"-Rep-",Id_Rep)
                    KKT,CountF,CountG,CountH,Alpha,Time,FETri,FEOLDTri,NSRatio,FSRatio,Id_Con = AdapGDSQP(nlp,Sigma[Id_Sig],Kappa[Id_Kap],cerr,cgrad,Max_Iter,EPS_Step,EPS_Res,epsilon,delta,eta,beta,alpha_max,rho,kap_grad,kap_f,chi_grad,chi_f,p_grad,p_f,0)
                    if Id_Con != 1
                        Id_Rep += 1
                    else
        ...
        for Id_CE = 2:LenTest[3]
            for Id_Sig = 1:LenSigma
                Id_Rep = 1
                while Id_Rep <= Rep
                    println("AdapGD-",Id_Prob,"-CE-",Id_CE,"-Sig-",Id_Sig,"-Rep-",Id_Rep)
                    KKT,CountF,CountG,CountH,Alpha,Time,FETri,FEOLDTri,NSRatio,FSRatio,Id_Con = AdapGDSQP(nlp,Sigma[Id_Sig],kap,chi_err[Id_CE],cgrad,Max_Iter,EPS_Step,EPS_Res,epsilon,delta,eta,beta,alpha_max,rho,kap_grad,kap_f,chi_grad,chi_f,p_grad,p_f,0)
                    if Id_Con != 1
                        Id_Rep += 1
                    else
        ...
        for Id_Sig = 1:LenSigma
            Id_Rep = 1
            while Id_Rep <= Rep
                println("AdapGD-",Id_Prob,"-MN-Sig-",Id_Sig,"-Rep-",Id_Rep)
                KKT,CountF,CountG,CountH,Alpha,Time,FETri,FEOLDTri,NSRatio,FSRatio,Id_Con = AdapGDSQP(nlp,Sigma[Id_Sig],kap,cerr,cgrad,Max_Iter,EPS_Step,EPS_Res,epsilon,delta,eta,beta,alpha_max,rho,kap_grad,kap_f,chi_grad,chi_f,p_grad,p_f,1)
                if Id_Con != 1
                    Id_Rep += 1
                else
        ...
        ## Save result
        AdapG[Id_Prob] = AdapGDResult(KKTStep,CountFStep,CountGStep,CountHStep,alphaStep,TimeStep,FETriStep,FEOLDTriStep,NSRatioStep,FSRatioStep)
        finalize(nlp)
    end
