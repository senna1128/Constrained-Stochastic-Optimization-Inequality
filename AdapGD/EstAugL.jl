...
function EstAugL(nlp,nx,necon,nicon,IdLow,IdUpp,eta,epsilon,nu,sigma,CovM,Xi_f,f_k,c_k,x,mulam,nabf_k,G_k,G_ktMuLam_k,q_nuxlam,omega_epsnu_xlam,alpha_k,BarSQP,BV,diagg2lam)

    # estimate f
    bf_k = f_k + rand(Normal(0,(sigma/Xi_f)^(1/2)))
    bnabf_k = mean(rand(MvNormal(nabf_k, CovM), convert(Int64, floor(Xi_f))),dims = 2)
    J_1 = G_k[1:necon,:]*(bnabf_k+G_ktMuLam_k)
    J_2 = G_k[IdUpp:end,:]*(bnabf_k+G_ktMuLam_k)+diagg2lam
